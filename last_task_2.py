...
def index():
    if 'name' in session:
        return render_template('last_task_2_login.html', name=str(session['name']))
    else:
...
@app.route('/new')
def new():
    return render_template('last_task_2_new.html')
...
def new_login():
    if request.form.get('newuser') and request.form.get('npw'):
        ...
        if len(rows) == 0:
            cur.execute('INSERT INTO list(id, pw) VALUES(%s, %s)',(new_user, new_pw))
        else:
            for item in rows[0]:
                if new_user == item:
                    return redirect('/new')
