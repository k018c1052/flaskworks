...
def index():
    if 'name' in session:
        ...
        rows = cur.fetchall()
        conn.close()
        return render_template('last_task_3_login.html', name=li_name, list=rows)
    else:
        return render_template('last_task_3.html')
...
@app.route('/new')
def new():
    return render_template('last_task_3_new.html')
